...
from LinkGrabberDjango.api.serializers import SnippetSerializer
from LinkGrabberDjango.forms import User, FeatureRequestForm
from LinkGrabberDjango.templatetags.links_extras import sslimage, urldecode
from .api import subscene
from .forms import UserForm, UserLoginForm
from .models import watched, Profile, favourite, FeatureRequest, Post


def post_list(request):
    pagename = "Search Results"
    return render(request, 'gui/menumain.html', {'posts' : range(40) , "pagename": pagename})
...
class ApiSearch(APIView):
    def get(self, request, format=None):
        ...
        if ('q' in request.GET) and request.GET['q'].strip():
            ...
            entry_query = apigrabber.get_query(query_string, ['title', ])

            found_entries = Post.objects.filter(entry_query).order_by('-rating')
        serializer = SnippetSerializer(found_entries, many=True)
        data = {
            "shows": [{'showid': item["id"], 'showname': item["title"], 'poster': item["poster"], 'year': item["year"],
                       'rating': item["rating"]} for item in
                      serializer.data]}
...
class getFavouritesAPI(APIView):
    """
    getCurrentUser Favs.
...
class getDetailsAPI(APIView):
    """
    Get Details and videos for listing.
    """
...
class getLinksAPI(APIView):
    ...
    def get(self, request, format=None):
        id = request.GET.get('id')
        links = apigrabber.getstreams(id)
        data = {
            "links": [
                {'id': item["id"], 'chapter_id': item["chapter_id"], 'name': item["name"], 'url': item["link"]} for item in links]}
        return Response(data)

class decodeGFileAPI(APIView):
    """
    Get Details and videos for listing.
    """
    def get(self, request, file, format=None):
        data = {"decoded": urldecode(file)}
        return Response(data)

class GetNextEpisodeAPI(APIView):
    """
    Gets next episode from episode id and show id
    """
...
class GetPrevEpisodeAPI(APIView):
    """
    Get Previous episode from episode and show id.
    """
